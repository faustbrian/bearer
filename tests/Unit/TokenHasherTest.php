...
describe('TokenHasherNotFoundException', function (): void {
    describe('Happy Path', function (): void {
        test('creates exception for hasher not found by name', function (): void {
            ...
            // Act
            $exception = TokenHasherNotFoundException::forHasher($name);

            // Assert
            expect($exception)->toBeInstanceOf(TokenHasherNotFoundException::class);
            expect($exception->getMessage())->toBe("Token hasher 'bcrypt' is not registered.");
        });

        test('handles different hasher names', function (): void {
            // Arrange & Act
            $exception1 = TokenHasherNotFoundException::forHasher('sha256');
            $exception2 = TokenHasherNotFoundException::forHasher('sha512');
            $exception3 = TokenHasherNotFoundException::forHasher('custom-hasher');
...
describe('NoDefaultTokenHasherException', function (): void {
    describe('Happy Path', function (): void {
        test('creates exception when no default hasher is set', function (): void {
            // Arrange & Act
            $exception = NoDefaultTokenHasherException::noDefault();

            // Assert
            expect($exception)->toBeInstanceOf(NoDefaultTokenHasherException::class);
