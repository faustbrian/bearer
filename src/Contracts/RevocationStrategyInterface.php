 ...
 * security models and use cases, from simple single-token revocation to complex
 * hierarchical invalidation.
 *
 * Common revocation scenarios: - Single token: Revoke only the specific token -
 * Token family: Revoke all tokens in a rotation chain - Token group: Revoke all
 * tokens in a logical group - User tokens: Revoke all tokens for a user -
 * Cascade revocation: Revoke tokens that depend on this token
 *
 * Strategies are particularly important for: - Security incidents requiring
 * broad token invalidation - User logout functionality (revoke current vs all
 * tokens) - Token rotation chains where old tokens must be invalidated -
 * Hierarchical token systems with parent-child relationships
 *
 * ```php class SingleTokenRevocation implements RevocationStrategyInterface {
...
interface RevocationStrategyInterface
{
     ...
     *
     * Implementations should: - Mark tokens as revoked (timestamp or boolean
     * flag) - Trigger any necessary cascade operations - Emit audit events for
     * revoked tokens - Handle edge cases (already revoked, expired, etc.) - Be
     ...
     * Previewing revocation impact before executing - Warning users about how
     * many tokens will be affected - Audit logging of all affected tokens - UI
     * display of revocation scope
     *
