 ...
 * enabling compliance, security monitoring, and debugging capabilities.
 * Different implementations can store audit data in databases, log files,
 * external services, or specialized audit platforms.
 *
...
interface AuditDriverInterface
{
     ...
     * The event contains structured information about what occurred, while the
     * context array provides additional request-specific or environmental data.
     *
     * Implementations should: - Store events durably and reliably - Include
     * timestamps automatically - Capture request metadata (IP, user agent) when
     * available - Handle failures gracefully without breaking the main flow
     *
     * The context array might include: - ip_address: Client IP address -
     * user_agent: HTTP user agent - request_id: Unique request identifier -
     * resource: Resource being accessed - action: Specific action attempted
     *
