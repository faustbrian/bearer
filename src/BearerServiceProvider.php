...
final class BearerServiceProvider extends PackageServiceProvider
{
    ...
    {
        $this->app->singleton(TokenTypeRegistry::class, function ($app): TokenTypeRegistry {
    ...
    {
        $this->app->singleton(TokenGeneratorRegistry::class, function ($app): TokenGeneratorRegistry {
            assert($app instanceof Application);
            ...
            foreach ($drivers as $name => $class) {
                ...
                if (array_key_exists($name, $builtInGenerators)) {
                    continue;
                }
    ...
    {
        $this->app->singleton(TokenHasherRegistry::class, function ($app): TokenHasherRegistry {
            assert($app instanceof Application);
            ...
            foreach ($drivers as $name => $class) {
                ...
                if (array_key_exists($name, $builtInHashers)) {
                    continue;
                }
    ...
    {
        $this->app->singleton(AuditDriverRegistry::class, function ($app): AuditDriverRegistry {
            assert($app instanceof Application);
            ...
            foreach ($drivers as $name => $config) {
                ...
                if (array_key_exists($name, $builtInDrivers)) {
                    continue;
                }
    ...
    {
        $this->app->singleton(RevocationStrategyRegistry::class, function ($app): RevocationStrategyRegistry {
            assert($app instanceof Application);
    ...
    {
        $this->app->singleton(RotationStrategyRegistry::class, function ($app): RotationStrategyRegistry {
            assert($app instanceof Application);
    ...
    {
        $this->app->singleton(BearerManager::class, function ($app): BearerManager {
            assert($app instanceof Application);
    ...
    /**
     * Register Bearer models with VariableKeys.
     *
     * Configures primary key types for Bearer models based on the
     * `bearer.primary_key_type` configuration setting.
     */
    private function registerVariableKeys(): void
    {
        ...
        VariableKeys::map([
            ...
            AccessTokenAuditLog::class => [
                'primary_key_type' => $primaryKeyType,
            ],
            AccessTokenGroup::class => [
                'primary_key_type' => $primaryKeyType,
            ],
        ]);
    }
