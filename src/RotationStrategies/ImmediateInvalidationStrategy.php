 ...
 *
 * Immediately revokes the old token when rotation occurs, making it invalid for
 * any further use. This is the most secure rotation strategy but requires
 ...
 * token reuse but may cause issues if the client fails to receive or store the
 * new token.
 *
 * Use this strategy when: - Security is paramount and token reuse must be
 * prevented - Clients can reliably receive and store new tokens - You're
 * willing to handle failed rotations (client didn't get new token) - The system
 * can tolerate brief authentication failures during rotation
 */
final class ImmediateInvalidationStrategy implements RotationStrategyInterface
{
     ...
     * revoked_at to the current timestamp. This ensures only one token is valid
     * at any time, providing maximum security at the cost of requiring atomic
     * token updates by clients.
     *
     ...
     * upon rotation, ensuring only the new token can be used for
     * authentication.
     *
     ...
     *
     * Returns null as this strategy does not use a grace period. Old tokens are
     * immediately invalidated with no transition period.
     */
