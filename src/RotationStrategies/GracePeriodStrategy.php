 ...
 *
 * Allows the old token to remain valid for a specified grace period after
 * rotation, giving clients time to transition to the new token without
 * authentication failures. This balances security with reliability by
 * preventing immediate token invalidation.
 *
 * The grace period is implemented by setting the revoked_at timestamp to a
 * future time. During this period, both the old and new tokens are valid. After
 * the grace period expires, only the new token will authenticate successfully.
 *
 * This strategy is ideal for production environments where you need to ensure
...
final readonly class GracePeriodStrategy implements RotationStrategyInterface
{
     ...
     * remain valid, allowing clients to gracefully transition without downtime.
     *
     ...
     *
     * Returns true if the token has not been revoked or if the revocation
     * timestamp is in the future (within the grace period). Once the grace
     * period expires, this method returns false and the token becomes invalid.
     *
