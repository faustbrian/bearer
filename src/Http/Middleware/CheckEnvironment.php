 ...
 * matches one of the allowed environments (e.g., 'test', 'live',
 * 'development'). This enables separation between production and
 * test/development API access.
 *
 * Common use cases: - Prevent test tokens from accessing production endpoints -
 * Restrict certain operations to live environment only - Allow sandbox testing
 * with test environment tokens
 *
 * Usage in routes: ```php Route::post('/payments/charge', function () {
...
final class CheckEnvironment
{
     ...
     * access token (not session-based auth) 3. The token's environment matches
     * one of the allowed environments
     *
     ...
     * @throws EnvironmentNotAllowedException If the token environment doesn't match any allowed environment
     *
    ...
    public function handle(Request $request, Closure $next, string ...$environments): mixed
    {
        ...
        if (!in_array($currentEnvironment, $environments, true)) {
            throw EnvironmentNotAllowedException::notAllowed($currentEnvironment, $environments);
        }
