...
trait HasBearerPrimaryKey
{
     ...
     *
     * @throws NonStringUuidException When a UUID value is not a string
     * @throws NonStringUlidException When a ULID value is not a string
     */
    #[Boot()]
    protected static function generatePrimaryKey(): void
    {
        static::creating(function (Model $model): void {
            ...
            if ($primaryKey->type === PrimaryKeyType::UUID && !is_string($existingValue)) {
                throw NonStringUuidException::nonStringUuid($existingValue);
            }

            // Validate manually set ULID values are strings
            if ($primaryKey->type === PrimaryKeyType::ULID && !is_string($existingValue)) {
                throw NonStringUlidException::nonStringUlid($existingValue);
            }
        });
    }
}
